...
uas_parser = UASparser('/path/to/your/cache/file')
...
class UASparser:
    ...
    def __init__(self, cache_file_name=None, update_interval=3600*24*10): # 10 day update interval
        """
        ...
        if self.cache_file_name == None:
            self.cache_file_name = os.path.abspath( os.path.dirname(__file__) + "/uasparser.cache" )
        if not os.path.exists(self.cache_file_name) or update_interval != None:
            if os.path.exists(self.cache_file_name) and not os.access(self.cache_file_name, os.W_OK):
                raise UASException("Cache file %s is not writable." % self.cache_file_name)
            elif os.path.exists(self.cache_file_name) and not os.access(os.path.dirname(self.cache_file_name), os.W_OK):
                raise UASException("Cache directory %s is not writable." % os.path.dirname(self.cache_file_name))
    ...
    def _checkCache(self):
        ...
        if not os.path.exists(cache_file):
            ...
            if self.update_interval != None and mtime < time.time() - self.update_interval:
                return False
