...
def membermatch(memberids, user, myslate=False):
    ...
    def getbillvotes(slatebills):
        billvotes = []        
        for slatebill in slatebills:
            #determine if slatebills has tuples (for slates) or comments (for congress match), and identify the bill accordingly
            if type(slatebill) is tuple:
                thisbill = slatebill[0]
            elif type(slatebill) is popvox.models.UserComment:
                thisbill = slatebill.bill
            else:
                #in case someone passes something weird into this function:
                print type(slatebill)
                continue
            
            #grabbing bill info
            try:
                dom1 = popvox.govtrack.getBillMetadata(thisbill)
            except:
            ...
            for vote in allvotes:
                ...
                votexml = "/mnt/persistent/data/govtrack/us/" + str(thisbill.congressnumber) + "/rolls/"+where+year+"-"+roll+".xml"
            ...
            if len(memberids) > 1:
                if not had_vote:
                    billvotes.append( (slatebill, None) )
                    continue
                ...
                billvotes.append( (slatebill, voters_votes) )
    ...
    slatebills = []
    #myslate is a list of tuples of each bill and position from a given slate object (bill, '+') or (bill, '-')
    if myslate:
        for slatebill in myslate:
            print "slatebill:"+ str(slatebill)
            if not slatebill[0].is_bill(): continue
            slatebills.append(slatebill)
    #If there's no slate, make one of the bills the user has commented on (for congress match)
    else:
        ...
        for comment in usercomments:
            ...
            slatebills.append(comment)
            
    billvotes = getbillvotes(slatebills)
    ...
    if not myslate: #sorting congress match only, not bill slates
    ...
    for id in memberids: # init each member to zeroes
        if id == popvox.govtrack.CURRENT_LEADERSHIP[0]: # we need to know if it's the Speaker of the House because they don't vote much
            stats.append({ "agree": 0, "disagree": 0, "0": 0, "P": 0, "_TOTAL_": 0, "is_speaker": "True" })
        else:
            stats.append({ "agree": 0, "disagree": 0, "0": 0, "P": 0, "_TOTAL_": 0, "is_speaker": "False" })
    for slatebill, votes in billvotes: # for each vote...
        if not votes: continue # no vote on this bill
        #checking slatebill type to separate comments from slate tuples:
        if type(slatebill) is tuple:
            position = slatebill[1]
        else:
            position = slatebill.position
