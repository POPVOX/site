...
def getMembers():
    ...
    for abbr in stateabbrs:
        ...
        if stateapportionment[abbr] == 1:
            statemems = getMembersOfCongressForDistrict(abbr, "all")
            for mem in statemems:
                members[abbr].append(mem["name"])
            continue
        else:
            senators = getMembersOfCongressForDistrict(abbr, "sen")
            for sen in senators:
                members[abbr].append(sen["name"])
        for d in xrange(stateapportionment[abbr]):
            try:
                members[abbr].append( getMembersOfCongressForDistrict(abbr + str(d+1), "rep")[0]["name"] )
            except:
                members[abbr].append("vacant")
    return members
