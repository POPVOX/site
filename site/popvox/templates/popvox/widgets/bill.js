...
document.write('</style>');
document.write('<div class="popvoxwidget {% if request.GET.title == "1" %}wide{% else %}small{% endif %}" onclick="document.location=\'{{siteroot|escapejs}}{% if bill %}{{bill.url|escapejs}}{% endif %}\'"><div class="procon">{% if stats %}{{stats.pro_pct}}%<br/>{{stats.con_pct}}%{% endif %}</div>' +
