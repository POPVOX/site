...
def district_lookup_coordinate(lng, lat):
	...
	cursor = connection.cursor()
	cursor.execute("SELECT state, district FROM congressionaldistrictpolygons WHERE ST_Contains(bbox, GeomFromText('Point(%s %s)'))", [float(lng), float(lat)])
	rows = cursor.fetchall()
...
def county_lookup_coordinate(lng, lat):
	# Find all of the districts whose bounding box contains the point.
	cursor = connection.cursor()
	cursor.execute("SELECT state, name FROM countypolygons WHERE ST_Contains(bbox, GeomFromText('Point(%s %s)'))", [float(lng), float(lat)])
	rows = cursor.fetchall()
